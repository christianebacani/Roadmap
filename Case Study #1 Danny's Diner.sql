...
WITH cte AS (SELECT 
				*,
				ROW_NUMBER() OVER (PARTITION BY sub.customer_id
									ORDER BY purchased_count DESC) AS popular_product_rank
FROM (SELECT 
		customer_id,
		product_id,
		COUNT(product_id) AS purchased_count
	  ...
	  ORDER BY purchased_count DESC) AS sub -- Subquery using From Clause of the total number of orders per product, and customer
ORDER BY popular_product_rank)
...
SELECT
	*,
    ROW_NUMBER() OVER (PARTITION BY customer_id
						ORDER BY join_date) AS join_date_sort
FROM sales
INNER JOIN members
	USING(customer_id)
ORDER BY customer_id, join_date_sort) AS sub
...
WHERE join_date_sort = 4;
 ...
 SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY sales.customer_id
						ORDER BY sales.order_date) transaction_history_asc
FROM sales
JOIN members
	ON members.customer_id = sales.customer_id
WHERE (sales.customer_id = 'A' AND order_date < '2021-01-07') OR 
...
INNER JOIN menu
	USING(product_id)
WHERE customer_id = 'A' AND order_date <= '2021-01-31'
GROUP BY customer_id
UNION ALL
SELECT 
	customer_id,
...
INNER JOIN menu
	USING(product_id)
WHERE customer_id = 'B' AND order_date <= '2021-01-31'
GROUP BY customer_id;
